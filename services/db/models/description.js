...
Description.methods.getInfoPet = async () => {
  try {
    const queyCard =
      "SELECT * FROM mascota WHERE id_mascota = IDMASCOTA JOIN IMAGEN DE LA MASCOTA";
    const validation = Validation.methods.isQueryResponseEmpty(queyCard);
    return validation;
  } catch (error) {
    return Validation.methods.newError(
      "Error en la consula",
      "Hubo un error al consultar la descrpcion de la mascota"
    );
  }
};
